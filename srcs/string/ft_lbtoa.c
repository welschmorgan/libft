...
static unsigned		ft_lbtoa_len(t_llbtoa *n)
{
	n->len = 0;
	n->div = n->num;
	n->mod = n->div % n->base;
...
static t_llbtoa		*ft_lbtoa_init(t_llbtoa *n,
								   long num,
								   unsigned base)
{
...
static t_llbtoa		*ft_lbtoa_init_buf(t_llbtoa *n,
									   char *buf,
									   size_t max)
{
	n->max = buf ? max : n->len;
...
static t_llbtoa		*ft_lbtoa_do(t_llbtoa *n)
{
	n->div = n->num;
	n->mod = n->div % n->base;
	if (!n->num)
...
char				*ft_lbtoa_s(long n, unsigned base, char *buf, size_t max)
{
	t_llbtoa		env;
	char			*ret;

	ft_lbtoa_init(&env, n, base);
	ft_lbtoa_init_buf(&env, buf, max);
	if (!env.buf)
		return (env.buf);
	ft_lbtoa_do(&env);
	if (!buf)
